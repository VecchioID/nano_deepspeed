...
- 大小: `2 × Ψ × dtype_bytes` (m) + `2 × Ψ × dtype_bytes` (v) = **8Ψ** (fp32)
- 如果参数是 fp16，优化器状态通常用 fp32 维护，所以是 **8Ψ** (额外)
...
**模型状态** (无 ZeRO):
```
12Ψ = 12 × 7 × 10⁹ × 2 bytes ≈ 168 GB ← 单卡明显不够
```
...
- 但由于模型状态已经 168 GB，activation 的可变范围只是冰山一角
- **真正的内存大头在模型状态**，这就是 ZeRO 切分模型状态的动机
...
- 显存大头在哪? → 模型状态 (12Ψ) → 这就是 ZeRO 优化的目标
